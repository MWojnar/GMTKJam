...
public class GMTKJamRenderer extends GameRenderer {
	
	public static int timeSinceStart = 0;
	
	public GMTKJamRenderer(GameWorld world, int gameWidth, int gameHeight) {
	...
	public void render(float delta, float runTime) {
		...
		if (AssetLoader.assetManager.update()) {
			...
			if (((GMTKJamWorld) getMyWorld()).isPaused()) {
			...
			if (((GMTKJamWorld) getMyWorld()).isLoading()) {
	...
	//		}
			...
			if (((GMTKJamWorld) getMyWorld()).getLevelToLoad() != null) {
